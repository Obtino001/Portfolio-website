/**
 * Yasir Malik Portfolio — Project Detail Modal Web Component
 * <project-modal>
 */
...
class ProjectModal extends HTMLElement {
  ...
  initModal() {
    ...
    this.closeBtn = backdrop.querySelector('#pdm-close-btn');
    this.catEl = backdrop.querySelector('#pdm-cat');
    this.timeEl = backdrop.querySelector('#pdm-time');
    this.titleEl = backdrop.querySelector('#pdm-title');
    this.clientEl = backdrop.querySelector('#pdm-client');
    this.metricsEl = backdrop.querySelector('#pdm-metrics');
    this.challengeEl = backdrop.querySelector('#pdm-challenge');
    this.solutionEl = backdrop.querySelector('#pdm-solution');
    this.deliverablesEl = backdrop.querySelector('#pdm-deliverables');
    this.techEl = backdrop.querySelector('#pdm-tech');
    this.ctaBtn = backdrop.querySelector('#pdm-cta-btn');
  ...
  open(key) {
    ...
    if (this.titleEl) this.titleEl.textContent = data.title || data.client;
    if (this.clientEl) this.clientEl.textContent = data.client + ' — ' + (data.metric || '');
    if (this.challengeEl) this.challengeEl.textContent = data.challenge || '';
    if (this.solutionEl) this.solutionEl.textContent = data.solution || '';

    if (this.metricsEl) {
      this.metricsEl.innerHTML = (data.results || []).map(r => `
    ...
    if (this.deliverablesEl) {
      this.deliverablesEl.innerHTML = (data.deliverables || []).map(d => `<li>${d}</li>`).join('');
    }

    if (this.techEl) {
      this.techEl.innerHTML = (data.techStack || []).map(t => `<span class="pdm-tag">${t}</span>`).join('');
    }
